		...
		function userSearch(idx) {
			$.ajax({
				...
				success : function(res) {
					let str = res.split("/");
					$("#idx").val(str[0])
					$("#mid").val(str[1]).attr("readonly",true);
					$("#name").val(str[2])
					$("#age").val(str[3])
					$("#address").val(str[4])
...
<jsp:include page="/include/header.jsp"></jsp:include>
	<p><br /></p>
	<div class="container">
		<h2 class="text-center">AJax를 이용한 CRUD 구현하기</h2>
		<form>
			<table class="table table-bordered">
				...
				<tr>
					<td colspan="2" class="text-center">
						...
						<input type="reset" value="다시 입력" onclick="$('#mid').attr('readonly',false)" class="btn btn-info mr-2" />
						<input type="button" value="user수정" onclick="userUpdate()" class="btn btn-primary mr-2" />
						<input type="button" value="전체보기" onclick="location.href='userList.us';" class="btn btn-secondary mr-2" />
					</td>
				</tr>
			</table>
		...
		<table class="table table-hover text-center" >
			...
			<c:forEach var="vo" items="${vos}" varStatus="st">
				<tr>
					...
					<td>${vo.address}</td>
					<td>
						<a href="javascript:userSearch(${vo.idx})" class="btn btn-warning btn-sm" >개별 조회</a>
