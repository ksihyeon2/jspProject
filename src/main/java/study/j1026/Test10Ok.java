...
//				가입 조건과 맞지 않을때 다시 가입창으로 전달 이동
				response.sendRedirect(request.getContextPath()+"/study/1026/test10.jsp?flag=no");
			}	else {
//				가입 조건을 충족해 DB에 저장시키고 회원 main(test10Res.jsp)으로 이동
...
//				백엔드에 와서 처리 하고 바로 해당 경로로 감
				...
				response.sendRedirect(request.getContextPath()+"/study/1026/test10Res.jsp?name="+name+"&age="+age+"&gender=&hobby&job");
